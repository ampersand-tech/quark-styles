...
import * as DOMClassManager from './domClassManager';
import { Stash, StashOf } from './domClassManager';
import { safeAreaSize, registerSafeAreaDependent } from './safeArea';

import * as color from 'color';


const quarkStyles = DOMClassManager.startModule(module);
const QAC = quarkStyles.addClass;

let gColorConstants: StashOf<color> = {};
let gHasColorConstants = false;


function defaultSuffix(size: string, suffix: string): string {
...
registerSafeAreaDependent(calcSafeAreaLengthMAP);

function evalPixels(size: string): string {
...
function applyDirectionalStyles(style, stylePrefix, styleSuffix, key, size) {
  ...
  for (let i = 0; i < sides.length; ++i) {
    const styleName = stylePrefix + POS_LOOKUP[sides[i]] + styleSuffix;
...
function applyCornerDirectionalStyles(style: Stash, stylePrefix: string, styleSuffix: string, key1: string, key2: string, size: string) {
  let sides: string[] = [];
  if (!key1) {
    sides = ['tl', 'tr', 'bl', 'br'];
  } else if (key2) {
  ...
  for (let i = 0; i < sides.length; ++i) {
    const styleName = stylePrefix + CORNER_LOOKUP[sides[i]] + styleSuffix;
    style[styleName] = size;
  }
}
...
QAC(/^bxshdw-(-?[^-\n\r]+)-(-?[^-\n\r]+)-([^-\n\r]+)-(-?[^-\n\r]+)-([^-\n\r]+)-?([^-\n\r]+)?$/, function(match, style, errors) {
  const col = colorFromName(match[5], errors);
  let styleVal =
    evalPixels(match[1]) + ' ' +
    evalPixels(match[2]) + ' ' +
    evalPixels(match[3]) + ' ' +
...
// txtshdw
QAC(/^txtshdw-(-?[^-\n\r]+)-(-?[^-\n\r]+)-(-?[^-\n\r]+)-(.+)$/, function(match, style, errors) {
  const col = colorFromName(match[4], errors);
  const styleVal =
    evalPixels(match[1]) + ' ' + evalPixels(match[2]) + ' ' + evalPixels(match[3]) + ' ' +
    (col ? col.rgb().string() : '#000');
...
export function translateX(style, size) {
  if (!style.WebkitTransform && !style.transform) {
    style.WebkitTransform = style.transform = '';
  }
...
export function translateY(style, size) {
  if (!style.WebkitTransform && !style.transform) {
    style.WebkitTransform = style.transform = '';
  }
...
QAC(/^tkx-(.+)$/, function(match, style) {
  const size = defaultSuffix(match[1], 'deg');
  skewX(style, size);
});
 ...
 * Transform: scale(n)
 */
export function scale(style, size) {
  if (!style.WebkitTransform && !style.transform) {
    style.WebkitTransform = style.transform = '';
  }
...
QAC(/^us-([nta])$/, function(match, style) {
  const styleVal = USERSELECT_LOOKUP[match[1]];
...
QAC(/^trans-(.*)-([.0-9]+)s?$/, function(match, style) {
  ...
  for (let i = 0; i < properties.length; ++i) {
    const property = TRANSITION_LOOKUP[properties[i]];
...
QAC(/^c-(.[^-]+)-(.+)$/, function(match, style, errors) {
  let col = colorFromName(match[1], errors);
...
QAC(/^grad-([vhdc])-(.[^-]+)-(.[^-]+)(-b)?$/, function(match, style, errors) {
  const dir = match[1];
  const borderImage = match[4];
  const color1 = colorFromName(match[2], errors);
  const color2 = colorFromName(match[3], errors);
  if (!color1 || !color2) {
    errors && errors.push({
      err: 'invalid color specified in quarkStyle, see colorConstants',
      details: match[0],
  ...
  let dest: string = dir === 'v' ? 'bottom' : 'right';
  switch (dir) {
    case 'v': dest = 'to bottom';
      break;
    case 'h': dest = 'to right';
      break;
    case 'd': dest = '135deg';
      break;
    case 'c': dest = '45deg';
  ...
  const img = 'linear-gradient(' + dest + ' , ' + color1.rgb().string() + ' 0%, ' + color2.rgb().string() + ' 100%)';
...
QAC(/^distgrad-([vhdc])-(.*)/, function(match, style, errors) {
  ...
  for (let i = 0; i < props.length; i += 2) {
    const propColor = colorFromName(props[i], errors);
    if (!propColor) {
      errors && errors.push({
        err: 'invalid color specified in quarkStyle, see colorConstants',
        details: props[i],
  ...
  const img = `linear-gradient( ${dest}  , ${colorPairs.join(', ')} )`;
  style.backgroundImage = img;
  style.backgroundRepeat = 'no-repeat';
});

export function setColorConstants(colorConstants: StashOf<color>) {
...
QAC(/^invert-(.+)$/, function(match, style) {
  const amount = defaultSuffix(match[1], '%');
...
QAC(/^fadeIn-(.+)$/, function(match, style) {
  const amount = match[1] + 's';
  const anim = 'fadeIn ' + amount + ' ease-in';
